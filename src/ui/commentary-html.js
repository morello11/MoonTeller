...
const SQUARE_VOICE = 'sert'; // Sert Uygulama'nın mührü kare (tek istisna; kimlik)
...
export function sealHtml(state, size = '', key = voiceKey(state)) {
  const cls = ['muhur', size, key === SQUARE_VOICE ? 'sert' : ''].filter(Boolean).join(' ');
...
export function leafShell(state, target, focus, withNotch) {
  ...
  return `<div class="yaprak${withNotch ? ' centik' : ''}" data-state="yaziyor" data-target="${esc(target)}" data-focus="${esc(focus)}" tabindex="-1">`
    + `<div class="yaprak-bas">${sealHtml(state, 'orta')}<button type="button" class="imza"><span class="imza-ad">${esc(v.name)}</span></button>`
    ...
    + `<div class="yaprak-alt"><button type="button" class="negordu"><span class="i">i</span> ${esc(state.bank.copy('yorumcu_negordu'))}</button><span class="damga"></span></div><dl class="gordu"></dl></div>`;
}
...
export function leafBody(text) {
  ...
  const m = paragraphs[0].match(/^(.{12,160}?[.!?…])\s+(?=[A-ZÇĞİÖŞÜ"“(])(.*)$/s); // "2. ev" gibi kısaltmalarda bölme: sonraki cümle büyük harfle başlamalı
...
export function followupButtons(state, used = 0) {
  ...
  return LLM.followups.map(([key, label]) => `<button type="button" data-devam="${key}">${esc(label)}</button>`).join('');
}
...
export function pickerHtml(state) {
  ...
  const rows = LLM.voices.map((key) => {
    ...
    return `<button type="button" class="ses${key === current ? ' suanki' : ''}" data-ses="${key}"><span class="alinti">${esc(v.sample)}</span>`
